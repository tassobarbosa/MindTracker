---
description: Scan available GitHub Copilot agents and assign them to tasks in tasks.md
scripts:
...
2. **Scan Agent Definitions**: Discover all available GitHub Copilot agent definition files following the official hierarchy. Higher-priority levels override lower-priority agents with the same name.

   **Scanning priority order** (highest to lowest):
   1. **Project-level**: `.github/agents/*.md` in the repository root
   2. **User-level**: `~/.github/agents/*.md` in the user's home directory
   ...
   If no agent definition files are found at any level, **STOP** and report: "No agent definition files found at any hierarchy level (`.github/agents/` or `~/.github/agents/`). Since there are no specialized agents available, it is recommended to use `/speckit.implement` directly for task execution."
